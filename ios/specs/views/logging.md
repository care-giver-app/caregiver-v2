...
- **Module:** ios
- **Status:** Figma design pass **done** (wizard section `84:2`); spec written 2026-07-01 to document the already-built frames and record coherence decisions — next is the SwiftUI build.
...
- **Single-tracker quick-log** — no "log just this tracker" entry from a Home snapshot card or [[trackers]]
  row; the wizard is the only path.
- **Loading / error / partial-failure states** for the fan-out of POSTs are undesigned in Aurora.
