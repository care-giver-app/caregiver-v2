...
- **Module:** ios
- **Status:** **Built** — SwiftUI 2-step wizard (choose template → configure, per-number-field thresholds), 2026-07-09. Figma design pass done.
...
| 11  | Real templates vs illustrative              | Render whatever `GET /tracker-templates` returns — the real seed is **Weight · Blood Pressure · Temperature · Medication · Meal · Mood** (`templates.json`). Seed `icon` values are already **SF Symbols** (`heart`,`scalemass`,`pills`,…) → used directly on the card. | Build-phase 2026-07-09: the "Pain/Walk/Sleep" list above was illustrative Figma content; the real catalog differs. Superseded by this row.                                                                                                                    |
| 12  | Card hue + default color                    | Map template → Aurora hue (BP cyan · Weight teal · Medication violet · Temperature info-blue · Meal teal · Mood violet; unknown → cyan). Picker offers **[cyan, teal, violet, info-blue]**; the picked hue's **hex → stored `TrackerWrite.color`**.                     | Build-phase 2026-07-09: seed colors are **old-palette** (red/amber/green) and tiles render `tracker.color`, so the Aurora pick must override the seed hex (decision 9).                                                                                       |
| 13  | Threshold editor scope                      | An editable **MIN/MAX row for every `number` field** (prefilled from the template threshold; a blank bound = no limit). Non-number fields are read-only with no alert.                                                                                                  | Build-phase 2026-07-09 (Trevor): lets you add an alert to any numeric field (e.g. BP Pulse), not only ones the template pre-bounded. Extends decision 8.                                                                                                      |
| 14  | Custom card behaviour                       | Custom card **visible**; tapping shows a "coming soon" note — the blank-config screen stays deferred (decision 5).                                                                                                                                                      | Build-phase 2026-07-09: keep the escape-hatch discoverable without building it yet.                                                                                                                                                                           |
| 15  | Free color picker (**revises 9**)           | Configure offers the 4 Aurora hues as **quick-picks** _plus_ a native `ColorPicker` for **any** color. Default = the template's Aurora hue; the picked color is stored as hex (`Color.hexRGB`) in `TrackerWrite.color`.                                                 | Build-phase 2026-07-09 (Trevor): 4 choices felt too restrictive. Trade-off: a user _can_ now pick amber/red, which decision 9 excluded to protect recency/breach luminance — accepted for flexibility; revisit if it reads confusingly against status colors. |

> Decisions 11–15 are **SwiftUI build-phase** choices (2026-07-09); product decisions 1–10 came from the 2026-07-01 Figma brainstorm.
