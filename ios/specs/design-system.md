...
screens): the existing `AppLogo` asset (dark navy mark, **260pt wide, width-only frame** —
`scaledToFit` derives the height from the mark's own aspect so the plaque hugs the logo; 4pt
padding is the only whitespace. Enlarged + tightened from Figma's 200×82 + 16/12 padding on
2026-07-05, Trevor's call; _code leads Figma_, resize `Stride/Brand` next Figma pass) on a
near-white "ice chip" slab —
